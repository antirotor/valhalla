...
class PropertiesWindow(QtWidgets.QWidget):
    ...
    primitive type (like float, int, str, etc.) will be editable. When connected, editing will be
    disabled as values are then defined by connecting node.
    """
    ...
    def _process_table(self):
        ...
        self._table.setItem(pos, 1, TableWidgetHeaderItem(self._node.getName()))
        pos += 1
        self._table.insertRow(pos)
        self._table.setItem(pos, 0, TableWidgetHeaderItem("category"))
        self._table.setItem(pos, 1, QtWidgets.QTableWidgetItem(self._node._parent.get_category()))
        pos += 1
        self._table.insertRow(pos)
        self._table.setItem(pos, 0, TableWidgetHeaderItem("type uuid"))
        self._table.setItem(pos, 1, QtWidgets.QTableWidgetItem(self._node._parent.id))